describe('application.pages', function () {
    ...
    describe('applicationBrand directive', function () {
        ...
        describe('when config param has no value', function () {
            beforeEach(function () {
                configReaderDeferred.reject();
                scope.$digest();
            });

            it('do not show brand name', function () {
                expect(scope.brandNameVisible).toBeFalsy();
            });

            it('request logo image path', function () {
            ...
            it('logo path is on scope', function () {
                getImagePathDeferred.resolve('image-path');
                scope.$digest();

                expect(scope.logoSrc).toEqual('image-path');
            });
        });

        describe('when config param has value', function () {
            describe('and brand name is not visible', function () {
                beforeEach(function () {
                    configReaderDeferred.resolve({data: {value: 'false'}});
                    scope.$digest();
                });
            ...
            describe('and brand name is visible', function () {
                beforeEach(function () {
                    configReaderDeferred.resolve({data: {value: 'true'}});
                    scope.$digest();
                });

                it('read brand name translation', function () {
                ...
                it('do not request logo image path', function () {
                    expect(imageManagement.getImagePath).not.toHaveBeenCalled();
                });

                it('logo path is not on scope', function () {
                    scope.$digest();

                    expect(scope.logoSrc).toBeUndefined();
                });

                describe('and no translation available', function () {
                    beforeEach(function () {
                        i18nResolveDeferred.reject();
                        scope.$digest();
                    });

                    it('default to app namespace', function () {
                        expect(scope.brandName).toEqual('namespace');
                    });
                });

                describe('and with translation', function () {
                    beforeEach(function () {
                        i18nResolveDeferred.resolve('brand name');
                        scope.$digest();
                    });

                    it('brand name is available on scope', function () {
                        expect(scope.brandName).toEqual('brand name');
                    });
                });
            });
        });
